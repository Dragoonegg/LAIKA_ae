...
int lake_AES_GCM_init_fns(struct AES_GCM_engine_ctx *d_engine, char *cubin_path) {
    ...
    // 初始化 CUDA 驱动（只需要初始化一次）
    if (!dev_initialized) {
        res = cuInit(0);
        if (res != CUDA_SUCCESS) {
            PRINT("[lake] Error: cuInit failed with error %d (CUDA_ERROR_INVALID_HANDLE=400)\n", res);
            PRINT("[lake] Possible causes: 1) CUDA driver not installed or incompatible version\n");
            PRINT("[lake]                   2) No GPU device available\n");
            PRINT("[lake]                   3) CUDA library not properly loaded in userspace\n");
            PRINT("[lake]                   4) Permission issues accessing GPU device\n");
            return -ENODEV;
        }
        dev_initialized = 1;
    }

    // 每次调用都需要获取设备和创建上下文（因为每个 d_engine 需要独立的上下文）
    res = cuDeviceGet(&d_engine->device, 0);
    if (res != CUDA_SUCCESS) {
        PRINT("[lake] Error: acquire GPU device 0 failed with error %d\n", res);
        return -ENODEV;
    }
