...
void ecryptfs_destroy_crypt_stat(struct ecryptfs_crypt_stat *crypt_stat)
{
	...
	//TODO crypto_free_tfm(crypt_stat->tfm);
...
{
	struct skcipher_request *ablk_req = NULL;
	...
	if (cipher_mode_code == ECRYPTFS_CIPHER_MODE_GCM
		...
		skcipher_request_set_callback(ablk_req,
			CRYPTO_TFM_REQ_MAY_BACKLOG | CRYPTO_TFM_REQ_MAY_SLEEP,
			extent_crypt_complete, &ecr);
	}	
	...
	if(cipher_mode_code == ECRYPTFS_CIPHER_MODE_GCM
		...
		if (op == ENCRYPT)
			rc = crypto_skcipher_encrypt(ablk_req);
		else 
			rc = crypto_skcipher_decrypt(ablk_req);
	}
...
int ecryptfs_encrypt_page(struct page *page)
{
	...
	num_extents = PAGE_SIZE / crypt_stat->extent_size;
...
int ecryptfs_init_crypt_ctx(struct ecryptfs_crypt_stat *crypt_stat)
{
	...
	ecryptfs_printk(KERN_DEBUG,
			"Initializing cipher mode [%s] [%s]; strlen = [%d]; "
			"key_size_bits = [%zd]\n", crypt_stat->cipher_mode,
...
int ecryptfs_new_file_context(struct inode *ecryptfs_inode)
{
	...
	cipher_mode_name_len = strlen(mount_crypt_stat->global_default_cipher_mode_name);
