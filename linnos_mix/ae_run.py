...
# Prefixes to keep (4 groups)
PREFIXES = (
    "linnos+0_CPU_batch_",
    "linnos+0_dGPU_batch_",
    "linnos+0_APU_PL_batch_",
    "linnos+0_APU_PK_batch_",
    "linnos+1_CPU_batch_",
    "linnos+1_dGPU_batch_",
    "linnos+1_APU_PL_batch_",
    "linnos+2_CPU_batch_",
    "linnos+2_dGPU_batch_",
    "linnos+2_APU_PL_batch_",
)


def set_file_permissions(file_path):
    """Set file permissions so regular users can read (if run via sudo)"""
    try:
        ...
        if original_user:
            try:
                ...
                # Change file ownership to original user
...
def process_log(fin, fout):
    ...
    for raw_line in fin:
        ...
        if "] " in line:
            ...
            # If unexpectedly no timestamp, use the whole line
            payload = line.strip()
        ...
        if m:
            batch = int(m.group(1))
            # 5) Drop the first batch=16 sample for each group (warmup)
...
def main():
    ...
    log_file.close()
    # Set file permissions so regular users can read
    ...
    # Pass workload return code through
