...
ecryptfs_base = os.path.dirname(this_path)
ecryptfs_dir = os.path.join(ecryptfs_base, "ecryptfs")
crypto_dir   = os.path.join(ecryptfs_base, "crypto")
crypto_nv_dir = os.path.join(ecryptfs_base, "crypto_nv")
fileio_dir   = os.path.join(ecryptfs_base, "file_io")
...
if not os.path.isfile(os.path.join(crypto_dir, "gcm_kernels.hsaco")):
    print(f"gcm_kernels.hsaco not found. run make in {crypto_dir}")
    sys.exit(1)

if not os.path.isfile(os.path.join(crypto_nv_dir, "lake_gcm.ko")):
    print(f"lake_gcm.ko not found in crypto_nv. run make in {crypto_nv_dir}")
    sys.exit(1)

if not os.path.isfile(os.path.join(crypto_nv_dir, "gcm_kernels.cubin")):
    print(f"gcm_kernels.cubin not found. run make in {crypto_nv_dir}")
    sys.exit(1)
...
def load_lake_crypto(aesni_fraction=0):
    ...
    hsaco = os.path.join(crypto_dir, "gcm_kernels.hsaco")
    r = run(f"sudo insmod {p} hsaco_path={hsaco} aesni_fraction={aesni_fraction}", shell=True)
    if r.returncode < 0: 
        print(f"Error {r.returncode} inserting mod {p}")
        sys.exit(1)
...
def load_lake_crypto_nv(aesni_fraction=0):
    run("sudo modprobe aesni_intel", shell=True)
    p = os.path.join(crypto_nv_dir, "lake_gcm.ko")
    cubin = os.path.join(crypto_nv_dir, "gcm_kernels.cubin")
    cubin_abs = os.path.realpath(cubin)
    r = run(f"sudo insmod {p} cubin_path={cubin_abs} aesni_fraction={aesni_fraction}", shell=True)
    if r.returncode < 0: 
        print(f"Error {r.returncode} inserting mod {p}")
        sys.exit(1)

def load_lake_crypto_nv_75aesni(aesni_fraction=75):
    run("sudo modprobe aesni_intel", shell=True)
    p = os.path.join(crypto_nv_dir, "lake_gcm.ko")
    cubin = os.path.join(crypto_nv_dir, "gcm_kernels.cubin")
    cubin_abs = os.path.realpath(cubin)
...
tests = {
    ...
    "LAKE": {
        "cryptomod_fn": load_lake_crypto_nv,
        "fsmod_fn": load_lake_ecryptfs,
        "mount_fn": mount_lakegcm,
        "mount_basepath": os.path.join(ROOT_DIR, "lake_nv")
    },
    "LAKE75aesni": {
        "cryptomod_fn": load_lake_crypto_nv_75aesni,
        "fsmod_fn": load_lake_ecryptfs,
        "mount_fn": mount_lakegcm,
        "mount_basepath": os.path.join(ROOT_DIR, "lake_nv")
    },
...
# 收集输出结果
output_lines = []
output_lines.append("," + ",".join(sizes.keys()))
for name in tests.keys():
    output_lines.append(f"{name}_rd, {','.join(results[name]['rd'])}")
    output_lines.append(f"{name}_wt, {','.join(results[name]['wt'])}")
...
# 保存到文件
log_file = os.path.join(this_path, "kernel.log")
